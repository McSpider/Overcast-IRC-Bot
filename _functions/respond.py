...
class function(function_template):
    ...
    def main(self, bot, msg_data, func_type):
        ...
        if re.match("^(hi|hello|hey),? %s.*?$" % re.escape(bot.irc.nick), message, re.IGNORECASE) or re.match("^%s[,:]? (hi|hello)$" % re.escape(bot.irc.nick), message, re.IGNORECASE):
            if self.checkCooldownForID(cooldown_id,1):
                bot.irc.sendMSG("%s %s" % (random.choice(self.greetings), msg_data["sender"]), msg_data["target"])
                self.addCooldownForID(cooldown_id,1)
                return True
        if re.match("^.*?how (do you do|are you) %s?" % re.escape(bot.irc.nick), message, re.IGNORECASE):
            if self.checkCooldownForID(cooldown_id,1):
                bot.irc.sendMSG("I'm good, thanks.", msg_data["target"])
                self.addCooldownForID(cooldown_id,1)
                return True
        if re.match("^\>_\<$", message):
            if self.checkCooldownForID(cooldown_id,2):
                bot.irc.sendMSG("Ouch!", msg_data["target"])
        ...
        if match:
            if self.checkCooldownForID(cooldown_id,3):
                ...
                if expression:
                    bot.irc.sendMSG(expression, msg_data["target"])
